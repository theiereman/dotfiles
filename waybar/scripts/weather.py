...
def print_help():
    ...
    print(' - \x1b[32mset-geocoding\x1b[0m <city[,state][,country]> : search and set coordinates in this script')
    print(' - \x1b[32mset-api-key\x1b[0m <api_key>            : update the API key in this script')
...
def make_request(call: str) -> list | dict:
    ...
    try:
        ...
        if req.status_code == 401:
            print_error('invalid API key. get a valid key at https://openweathermap.org/')
            quit(1)
        elif req.status_code != 200:
            print_error(f'API request failed with status code {req.status_code}')
            quit(1)
        ...
        print_error(f'failed to make request to `/{call}`: {str(e)}')
        quit(1)
...
def update_api_key(new_api_key: str):
    '''
    update the API_KEY constant in this script file.
    ...
    print(f'API key updated successfully in {script_path}')


def update_coordinates(latitude: float, longitude: float):
    '''
    update the LATITUDE and LONGITUDE constants in this script file.
    ...
    for i, line in enumerate(lines):
        if line.startswith('LATITUDE  = '):
            lines[i] = f"LATITUDE  = {latitude}"
        elif line.startswith('LONGITUDE = '):
            lines[i] = f"LONGITUDE = {longitude}"
    
    # write the updated content back to the file
    with open(script_path, 'w', encoding='utf-8') as file:
        file.write('\n'.join(lines))
    
    print(f'Coordinates updated successfully in {script_path}')
...
def geocoding(search: str) -> tuple[float, float] | None:
    '''
    ...
    @param search: search term in the format 'city[,state][,country]'

    @return: tuple of (latitude, longitude) if found, None otherwise
    '''
...
def set_geocoding(search: str):
    '''
    search for a city's coordinates and automatically update the LATITUDE and
    LONGITUDE constants in this script file.

    @param search: search term in the format 'city[,state][,country]'
    '''
    
    coords = geocoding(search)
...
def get_current_weather_data() -> dict:
    ...
    res = make_request(f'data/2.5/weather?lat={LATITUDE}&lon={LONGITUDE}&units=metric&lang=fr')
...
def get_forecast_data() -> dict:
    ...
    res = make_request(f'data/2.5/forecast?lat={LATITUDE}&lon={LONGITUDE}&units=metric&lang=fr')
...
def main():
    ...
    # >= 1 parameter provided
        ...
        if sys.argv[1] == 'geocoding':
            if len(sys.argv) == 3:
                if API_KEY is None:
                    print_error('API key not set. use `./weather.py set-api-key <key>` first')
                else:
                    geocoding(sys.argv[2])
            else:
                print_error('expected argument `<city[,state][,country]>`')
                print_help()
        # set geocoding (search and update coordinates)
        elif sys.argv[1] == 'set-geocoding':
            if len(sys.argv) == 3:
                if API_KEY is None:
                    print_error('API key not set. use `./weather.py set-api-key <key>` first')
                else:
                    set_geocoding(sys.argv[2])
            else:
                print_error('expected argument `<city[,state][,country]>`')
                print_help()
        # set api key (always available)
        elif sys.argv[1] == 'set-api-key':
            if len(sys.argv) == 3:
                update_api_key(sys.argv[2])
            else:
                print_error('expected argument `<api_key>`')
                print_help()
        # weather commands require both API key and coordinates
            ...
            if API_KEY is None:
                print_error('use `./weather.py set-api-key <key>` to set API key')
            if LATITUDE is None or LONGITUDE is None:
                print_error('use `./weather.py set-geocoding <city>` to set coordinates')
        # current weather
